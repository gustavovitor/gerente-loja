...
class UserBloc extends BlocBase {
  ...
  void onChangedSearch(String text) {
    if (text.trim().isEmpty)
      ...
      _usersController.add(_filter(text.trim()));
  }

  List<Map<String, dynamic>>_filter(String text) {
    List<Map<String, dynamic>> filteredUsers = List.from(_users.values.toList());
    filteredUsers.retainWhere((user) {
      return user["name"].toUpperCase().contains(text.toUpperCase());
    });
  ...
  void _subscribeToOrders(String uid) {
    _users[uid]["subscription"] = _firestore.collection("users").document(uid).collection("orders").snapshots().listen((orders) async {
      ...
      for (DocumentSnapshot document in orders.documents) {
